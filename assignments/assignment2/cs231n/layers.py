...
def layernorm_backward(dout, cache):
    ...
    dfdw = np.sum(dfdu * -1 / std, axis = 1) + dfdv * np.sum(-2/N * (x - mean), axis = 1)
    dx = dfdu / std + dfdv.reshape(-1, 1) * 2/N * (x - mean) + dfdw.reshape(-1, 1) / N
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            for m in range(H_out):
                for n in range(W_out):
                    index = np.argmax(x[i, j, m*stride:m*stride+PH, n*stride:n*stride+PW])
                    ind = np.unravel_index(index, (PH, PW))
                    dx[i, j, m*stride:m*stride+PH, n*stride:n*stride+PW][ind] = dout[i, j, m, n]
